...
python3 train.py \
        --port "${port}" \
        --exp-name "${encoder}_${crop_size}_fa_${flash_attn}_gc_${grad_ckpt}-1" \
        --project "vpt-aisystem-gckpt" \
...
python3 train.py \
        --port "${port}" \
        --exp-name "${encoder}_${crop_size}_fa_${flash_attn}_gc_${grad_ckpt}-2" \
        --project "vpt-aisystem-gckpt-gckpt" \
        --config-file configs/prompt/cub.yaml \
        --wandb \
        DATA.BATCH_SIZE "64" \
        DATA.CROPSIZE "${crop_size}" \
        ...
        OUTPUT_DIR "${output_dir}/seed${seed}" \
        MODEL.PROMPT.GRAD_CKPT "${grad_ckpt}" \
        MODEL.PROMPT.GRAD_CKPT_NUM_SEG "2"
...
python3 train.py \
        --port "${port}" \
        --exp-name "${encoder}_${crop_size}_fa_${flash_attn}_gc_${grad_ckpt}-3" \
        --project "vpt-aisystem-gckpt-gckpt" \
        --config-file configs/prompt/cub.yaml \
        --wandb \
        DATA.BATCH_SIZE "64" \
        ...
        OUTPUT_DIR "${output_dir}/seed${seed}" \
        MODEL.PROMPT.GRAD_CKPT "${grad_ckpt}" \
        MODEL.PROMPT.GRAD_CKPT_NUM_SEG "3"
...
python3 train.py \
        --port "${port}" \
        --exp-name "${encoder}_${crop_size}_fa_${flash_attn}_gc_${grad_ckpt}-4" \
        --project "vpt-aisystem-gckpt-gckpt" \
        --config-file configs/prompt/cub.yaml \
        --wandb \
        DATA.BATCH_SIZE "64" \
        ...
        OUTPUT_DIR "${output_dir}/seed${seed}" \
        MODEL.PROMPT.GRAD_CKPT "${grad_ckpt}" \
        MODEL.PROMPT.GRAD_CKPT_NUM_SEG "4"
