...
MODEL_ZOO = {
    ...
    "sup_vitb16_imagenet21k": "SUP-ViT-B.npz",
    ...
    "dino_vitb16" : "dino-ViT-B.pth",
    "beit_vitb16" : "beit-ViT-B.pth",
    "beitv2_vitb16" : "beitv2-ViT-B.pth",
    "simmim_swinb16" : "simmim-Swin-B.pth",
    "moby_swint16" : 'moby-Swin-T.pth'
}
...
def repeat_ckpt_sup(ckpt, index=0):
    new = OrderedDict()
    for k, v in ckpt.items():
        if 'block' in k:
            if int(k.split('/')[1].split('_')[-1]) != index:
                splits = k.split('/')
                splits[1] = 'encoderblock_{}'.format(index)
                key = '/'.join(splits)
                new[k] = ckpt[key]
            else:
                new[k] = v
