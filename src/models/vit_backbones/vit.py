...
ACT2FN = {"gelu": torch.nn.functional.gelu, "relu": torch.nn.functional.relu,
          "swish": swish, "fused_gelu" : fused_gelu}
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.config.flash_attn_torch20:
            with torch.backends.cuda.sdp_kernel(enable_flash=True, enable_math=False, enable_mem_efficient=False):
                context_layer = F.scaled_dot_product_attention(
                    query_layer, key_layer, value_layer, dropout_p=self.config.transformer["attention_dropout_rate"]
                )
                weights = None
        else:
        ...
        attention_output = self.out(context_layer) # aggregate multi-head attention 
...
class Embeddings(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        dim = x.shape[-1]
        w0 = w // self.patch_embeddings.kernel_size
        h0 = h // self.patch_embeddings.kernel_size
...
class Block(nn.Module):
    def __init__(self, config, vis):
        ...
        if config.flash_attn and not config.flash_attn_torch20:
            from flash_attn.flash_attention import FlashMHA
            self.attn = FlashMHA(
    ...
    def load_from(self, weights, n_block, fattn=False):
        ...
        with torch.no_grad():
            ...
            if not fattn:
                ...
                # self.attn.Wqkv.weight.copy_(qkv_weight)
                # self.attn.Wqkv.bias.copy_(qkv_bias)
                # self.attn.out_proj.weight.copy_(out_weight)
                # self.attn.out_proj.bias.copy_(out_bias)
                qkv_weight = torch.cat([query_weight, key_weight, value_weight], dim=0).half()
                qkv_bias = torch.cat([query_bias, key_bias, value_bias], dim=0).half()
                
                self.attn.Wqkv.weight.copy_(qkv_weight)
                self.attn.Wqkv.bias.copy_(qkv_bias)
                self.attn.out_proj.weight.copy_(out_weight.half())
                self.attn.out_proj.bias.copy_(out_bias.half())
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis):
        ...
        self.embeddings = Embeddings(config, img_size=img_size) # patch embedding # for better reference of ViT code, see timm
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.is_fattn = config.flash_attn
        ...
        with torch.no_grad():
            ...
            for bname, block in self.transformer.encoder.named_children():
                for uname, unit in block.named_children():
                    unit.load_from(weights, n_block=uname, fattn=self.is_fattn)
