...
class PromptedVisionTransformer(VisionTransformer):
    def __init__(
        ...
        vit_cfg.flash_attn = prompt_cfg.FLASH_ATTN
        vit_cfg.flash_attn_torch20 = prompt_cfg.FLASH_ATTN_TORCH20
        self.transformer = PromptedTransformer(
            prompt_cfg, vit_cfg, img_size, vis)

        if self.prompt_cfg.GRAD_CKPT:
            self.transformer_modules = [module for k, module in self.transformer._modules.items()]
            
    def forward(self, x, vis=False):
        if not self.prompt_cfg.GRAD_CKPT:
            # x, attn_weights = self.transformer(x)
            x = self.transformer(x)
        else:
            x.requires_grad = True
            x = checkpoint_sequential(functions=self.transformer_modules, 
                                                    segments=self.prompt_cfg.GRAD_CKPT_NUM_SEG,
                                                    input=x)
        x = x[:, 0]
