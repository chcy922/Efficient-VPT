...
def setup(args):
    ...
    # setup dist
    cfg.DIST_PORT = args.port
    # cfg.DIST_INIT_PATH = "tcp://{}:{}".format(os.environ["SLURMD_NODENAME"], cfg.DIST_PORT)
    ...
    fa = 'y' if cfg.MODEL.PROMPT.FLASH_ATTN_TORCH20 else 'n'
    gc = 'y' if cfg.MODEL.PROMPT.GRAD_CKPT else 'n'
    output_folder = os.path.join(
        cfg.DATA.NAME, cfg.DATA.FEATURE, f"{args.id}_lr{lr}_wd{wd}_fa_{fa}_gc_{gc}")
...
def train(cfg, args):
    ...
    trainer = Trainer(cfg, model, evaluator, cur_device, wandb)
